...
def _clasificar_fiabilidad(valor: float) -> str:
...
def generar_reporte_texto(df: pd.DataFrame, ruta_origen: str = "") -> str:
    ...
    for dimension, datos in r["fiabilidad"].items():
        lineas.append(
            ...
            f"alpha={datos['alpha']:.3f} [{_clasificar_fiabilidad(datos['alpha'])}]  "
            f"omega={datos['omega']:.3f} [{_clasificar_fiabilidad(datos['omega'])}]"
        )
    if not r["fiabilidad"]:
